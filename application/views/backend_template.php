...
<body>
    <div class="app-container app-theme-white body-tabs-shadow fixed-header fixed-sidebar">
        <?php $this->load->view('layout/header', true); ?>
        ...
        <div class="app-main">
            <div class="app-sidebar sidebar-shadow <?php echo theme('sidebar'); ?>">
                                    ...
                                    <div class="d-inline-block dropdown">
                                                ...
                                                <div tabindex="-1" role="menu" aria-hidden="true" class="dropdown-menu dropdown-menu-right">
                                                    <ul class="nav flex-column">
                                                        <?php foreach ($dropdown_option as $k_d => $v_d) { 
                                                        if ($v_d['tipe']=='link') {
                                                             $action_klik = 'href="'.base_url($v_d['onclick']).'"';
                                                         }else{
                                                             $action_klik = 'onclick="'.$v_d['onclick'].'"';

                                                         } ?>
                                                        <li class="nav-item">
                                                            <a class="nav-link" <?php echo $action_klik.' '.$v_d['elemen_tambahan'] ?>>
                                                                <i class="nav-link-icon <?php echo $v_d['fa'] ?> "></i>
                                                                <span>
                                                                    <?php echo ($v_d['caption']) ?>
                                                                </span>
...
<div class="modal fade" id="modal_loading" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog " role="document">
        <div class="modal-content">
          
            <div class="modal-body">
               <!-- Loading...<span class="titik_loading"></span> -->
               <div style="text-align: center;"> <b id="caption_loading" ></b></div>
    ...
    <script>
        ...
        function get_paket_ditolak() {
            $.ajax({
                url: baseUrl('backend_template/get_paket_ditolak'),
                type: 'POST',
                dataType: 'JSON',
                data: {},
                success: function(data) {
                    if (data.status == true) {
                        ...
                        $('#notif-paket-ditolak').html('');
                        $.each(data.data, function(k, v) {
                            $('#notif-paket-ditolak').append(`<div class="vertical-time-simple vertical-without-time vertical-timeline vertical-timeline--one-column">
                                <div class="vertical-timeline-item dot-danger vertical-timeline-element">
                                    <div>
                                        <span class="vertical-timeline-element-icon bounce-in"></span>
                                        <div class="vertical-timeline-element-content bounce-in">
                                            <h4 class="timeline-title">${v.nama_kegiatan}</h4>
                                            <a href="<?php echo base_url('realisasi/evidence/'); ?>${v.id}">${v.nama_paket} <strong>[ ${v.dokumen} ]</strong</a>
                                        </div>
                                    </div>
                                </div>
                            </div>`);
...
function start_loading(caption = ''){
    var show_caption = caption =='' ? 'Loading' : caption ; 
    $('#modal_loading').find('#caption_loading').html(show_caption);
    $('#modal_loading').modal({backdrop: 'static', keyboard: false});

    window.setTimeout(titik_loading(), 100);
}
