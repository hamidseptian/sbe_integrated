 ...
 protected function jumlah_sub_kegiatan($id_user)
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                if ($lists->jenis_paket=='RUTIN') {
                    ...
                    if (jadwal_rfk()['aktif']==0) {
                                    $row[]  = '<button class="btn btn-danger btn-sm" onclick="upload_berakhir()"><i class="fas fa-upload"></i></button>';
                                    $row[]  = '<button class="btn btn-info btn-sm" onclick="daftar_dokumen(' . "'" . sbe_crypt($lists->id_paket_pekerjaan, 'E') . "','".$izin_input_pptk ."','".$id_kedudukan ."'" . ')"><i class="fa fa-list-ul"></i></button>';
                                }else{

					                $nm_pk = str_replace('"', '', $lists->nama_paket);

                                    if ($izin_input_pptk==1) {
        	                            $row[]  = '<button class="btn btn-info btn-sm" onclick="upload_dokumen(' . "'" . $lists->id_instansi . "','" . 'null' . "','" . $lists->id_pptk . "','" . $lists->id_paket_pekerjaan . "','" . $lists->kode_rekening_sub_kegiatan . "','" . $nm_pk . "','" . $lists->jenis_paket . "','" . $lists->id_metode . "','" . $izin_input_pptk . "','" . $id_kedudukan . "'" . ')"><i class="fas fa-upload"></i></button>';
                                         $row[]  = '<button class="btn btn-info btn-sm" onclick="daftar_dokumen(' . "'" . sbe_crypt($lists->id_paket_pekerjaan, 'E') . "','".$izin_input_pptk ."','".$id_kedudukan ."'" . ')"><i class="fa fa-list-ul"></i></button>';


                                    }else{
                                        $row[]  = '<button class="btn btn-outline-danger btn-sm" onclick="'.$warning_bukan_kegiatan_anda.'"><i class="fas fa-upload"></i></button>';
                                        $row[]  = '<button class="btn btn-outline-danger btn-sm" onclick="'.$warning_bukan_kegiatan_anda.'"><i class="fa fa-list-ul"></i></button>';
    ...
    public function list_dokumen()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $dokumen = $this->db->query("SELECT * from master_bobot where jenis_paket='$jenis_paket' and id_metode='$id_metode' and nama_dokumen not in (SELECT dokumen from realisasi_fisik where id_paket_pekerjaan='$id_paket_pekerjaan')");
            ...
            if ($dokumen->num_rows() > 0) {
                ...
                $result_pelaksanaan = $this->db->query("SELECT * from vol_pelaksanaan_pekerjaan where id_paket_pekerjaan='$id_paket_pekerjaan' and id_vol_pelaksanaan_pekerjaan not in (SELECT id_vol_pelaksanaan_pekerjaan from realisasi_fisik where id_paket_pekerjaan='$id_paket_pekerjaan')");
                ...
                foreach ($dokumen_realisasi->result() as $key => $value) {
                    if (jadwal_rfk()['aktif']==0) {
                        ...
                        if ($value->auto_evidence==1) {
                             $tombol_edit_evidence =    ' <a href="#" class="btn btn-danger btn-sm" data-toggle="tooltip" title="upload ulang evidence" onclick="Swal.fire('."'warning','Dokumen ".$value->dokumen." tidak bisa diedit karena terintegrasi dengan SIPEDAL','error'".')"><i class="metismenu-icon fas fa-file-signature"></i></a>';
								...
								$tombol_hapus_evidence = ' <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Lihat Evidence evidence" target="_blank" onclick="Swal.fire('."'warning','Dokumen ".$value->dokumen." tidak bisa dihapus karena terintegrasi dengan SIPEDAL','error'".')"><i class="metismenu-icon fas fa-trash"></i></button>';
                        ...
                        $tombol_lihat_evidence =    ' <button class="btn btn-info btn-sm" data-toggle="tooltip" title="Lihat Evidence evidence" target="_blank" onclick="lihat_evidence('."'".$file_url."','".$nm_paket."','".$dokumen_evidence."'".')"><i class="metismenu-icon fas fa-folder-open"></i></button>';
                            ...
                            $tombol_hapus_evidence = ' <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Lihat Evidence evidence" target="_blank" onclick="hapus_evidence('."'".$value->id_realisasi_fisik."','".$nm_paket."','".$dokumen_evidence."'".')"><i class="metismenu-icon fas fa-trash"></i></button>';
                        }
                    }
                    ...
                    $output['data'][$key]['option']   =  $tombol_edit_evidence.$tombol_lihat_evidence.$tombol_hapus_evidence;
                    $output['status'] = true;
                }
    ...
    public function hapus_evidence_1_paket()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
    ...
    public function hapus_evidence()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $id_realisasi_fisik = $this->input->post('id_realisasi_fisik');//sbe_crypt($this->input->post('id_realisasi_fisik'), 'D');



            $data               = $this->db->get_where('realisasi_fisik', ['id_realisasi_fisik' => $id_realisasi_fisik])->row_array();
                $this->db->delete('realisasi_fisik', ['id_realisasi_fisik' => $id_realisasi_fisik]);
            ...
            //     // $output['id_pptk']  = $data['id_pptk'];
                $output['rekening'] = $data['kode_rekening_kegiatan'];
                $output['id_paket_pekerjaan'] = sbe_crypt($data['id_paket_pekerjaan'],'E');
            // }
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                $row[]  = $lists->nama_instansi;
                 ...
                 $realisasi_dipilih = $fisik_keuangan->cek_realisasi_dipilih($tahun_anggaran, $tahap, $id_instansi)->row_array();

                $realisasi_bo = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bo')->row_array()['realisasi_bo'];
                $realisasi_bm = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bm')->row_array()['realisasi_bm'];
                $realisasi_btt = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_btt')->row_array()['realisasi_btt'];
                $realisasi_bt = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bt')->row_array()['realisasi_bt'];
                // fusuk
                $realisasi_bo_rf = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bo')->row_array()['realisasi_bo_rf'];
                $realisasi_bm_rf = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bm')->row_array()['realisasi_bm_rf'];
                $realisasi_btt_rf = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_btt')->row_array()['realisasi_btt_rf'];
                $realisasi_bt_rf = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_bt')->row_array()['realisasi_bt_rf'];

                $realisasi_rf_total = $fisik_keuangan->total_realisasi_perjenis($bulan_aktif, $tahun_anggaran, $tahap, $id_instansi, 'realisasikan_fisik_total')->row_array()['realisasi_fisik_total'];
               ...
               $tombol_hapus = '<button class="btn btn-danger btn-xs"  title="Hapus realisasi keuangan sub kegiatan '.$lists->nama_sub_kegiatan.'"  onclick="'.$onclick_hapus.'"><i class="fas fa-trash"></i></button> ';
                ...
                // $hapus_data = $tombol_hapus;
                        ...
                        // </div> ';
                            ...
                            $forbidden_set_realisasi_bo = '<button type="button" class="tombol" style="color:red" title="Input Realisasi Belanja Operasi Sub  Kegiatan '.$lists->nama_sub_kegiatan.'"   onclick="'.$warning_bukan_kegiatan_anda.'">'.number_format($jumlah_realisasi_bo).'</button> ';
                            $forbidden_set_realisasi_bm = '<button type="button" class="tombol" style="color:red" title="Input Realisasi Belanja Operasi Sub  Kegiatan '.$lists->nama_sub_kegiatan.'"   onclick="'.$warning_bukan_kegiatan_anda.'">'.number_format($jumlah_realisasi_bm).'</button> ';
                            $forbidden_set_realisasi_btt = '<button type="button" class="tombol" style="color:red" title="Input Realisasi Belanja Operasi Sub  Kegiatan '.$lists->nama_sub_kegiatan.'"   onclick="'.$warning_bukan_kegiatan_anda.'">'.number_format($jumlah_realisasi_btt).'</button> ';
                            $forbidden_set_realisasi_bt = '<button type="button" class="tombol" style="color:red" title="Input Realisasi Belanja Operasi Sub  Kegiatan '.$lists->nama_sub_kegiatan.'"   onclick="'.$warning_bukan_kegiatan_anda.'">'.number_format($jumlah_realisasi_bt).'</button> ';



                                $row[] =  $realisasi_dipilih['realisasikan_bo']==0 ? '-' : $forbidden_set_realisasi_bo;
                                $row[] =  $realisasi_dipilih['realisasikan_bm']==0 ? '-' : $forbidden_set_realisasi_bm;
                                $row[] =  $realisasi_dipilih['realisasikan_btt']==0 ? '-' : $forbidden_set_realisasi_btt;
                                $row[] =  $realisasi_dipilih['realisasikan_bt']==0 ? '-' : $forbidden_set_realisasi_bt;
                               
                                $row[] = number_format($total_realisasi);
                                $row[] = $lists->pagu=='' ? 0 : number_format($lists->pagu);
                                $row[] = $pagu_total > 0 ? round($persentasi,2).' %' : '0 %';
                                $row[] = '<button class="btn btn-outline-danger btn-sm" onclick="'.$warning_bukan_kegiatan_anda.'">Terkunci</button>';
