...
class Data_apbd_model extends CI_Model
{
	...
	public function total_data_per_instansi($id_instansi, $tahap, $tahun)
	{
		if ($tahap == 4 ) {
			
			// $where 		=['id_instansi'=>$id_instansi, 'tahun'=>$tahun];
			$q_program = $this->db->query("SELECT count(DISTINCT kode_program) as jumlah_program from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status=1")->row_array();
			$program 	= $q_program['jumlah_program'];
			$q_kegiatan = $this->db->query("SELECT count(DISTINCT kode_kegiatan) as jumlah_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status=1")->row_array();
			$kegiatan 	= $q_kegiatan['jumlah_kegiatan'];
			$q_sub_kegiatan = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status='1'")->row_array();
			$q_sub_kegiatan_teknis = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status='1' and kategori='Unit Pelaksana'")->row_array();
			$q_sub_kegiatan_skpd = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status='1' and kategori='Sub Kegiatan SKPD'")->row_array();
		}else{
			// $where 		= ['id_instansi'=>$id_instansi, 'tahun'=>$tahun, 'kode_tahap'=>$tahap];
			$q_program = $this->db->query("SELECT count(DISTINCT kode_program) as jumlah_program from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'")->row_array();
			$program 	= $q_program['jumlah_program'];
			$q_kegiatan = $this->db->query("SELECT count(DISTINCT kode_kegiatan) as jumlah_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'")->row_array();
			$kegiatan 	= $q_kegiatan['jumlah_kegiatan'];
			$q_sub_kegiatan = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'")->row_array();
			$q_sub_kegiatan_teknis = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'and kategori='Unit Pelaksana'")->row_array();
			$q_sub_kegiatan_skpd = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'and kategori='Sub Kegiatan SKPD'")->row_array();
			
		}
	
			$sub_kegiatan 	= $q_sub_kegiatan['jumlah_sub_kegiatan'];
			$sub_kegiatan_teknis 	= $q_sub_kegiatan_teknis['jumlah_sub_kegiatan'];
			$sub_kegiatan_skpd 	= $q_sub_kegiatan_skpd['jumlah_sub_kegiatan'];
		...
		$data = [
			...
			'kegiatan'=>$kegiatan,
			'subkeg'=>$sub_kegiatan,
			'subkeg_teknis'=>$sub_kegiatan_teknis,
			'subkeg_skpd'=>$sub_kegiatan_skpd,
		
		];
		return $data;
	}

	public function total_data_per_instansi_expired($id_instansi)
	{
		$tahap 		= tahapan_apbd();
		...
		if ($tahap == 4 ) {
			...
			// $where 		=['id_instansi'=>$id_instansi, 'tahun'=>$tahun];
			$q_program = $this->db->query("SELECT count(DISTINCT kode_program) as jumlah_program from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status=0")->row_array();
			$program 	= $q_program['jumlah_program'];
			$q_kegiatan = $this->db->query("SELECT count(DISTINCT kode_kegiatan) as jumlah_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status=0")->row_array();
			$kegiatan 	= $q_kegiatan['jumlah_kegiatan'];
		}else{
			// $where 		= ['id_instansi'=>$id_instansi, 'tahun'=>$tahun, 'kode_tahap'=>$tahap];
			$q_program = $this->db->query("SELECT count(DISTINCT kode_program) as jumlah_program from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='$tahap'")->row_array();
		...
		$q_sub_kegiatan = $this->db->query("SELECT count(DISTINCT kode_sub_kegiatan) as jumlah_sub_kegiatan from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and status='0'")->row_array();
			$sub_kegiatan 	= $q_sub_kegiatan['jumlah_sub_kegiatan'];
	...
	// fungsi lama sebelum perubahan query
