...
class Data_apbd extends MY_Controller
{
    ...
    public function integrasi_sipd()
    {
        $breadcrumbs     = $this->breadcrumbs;
        $master_paket   = $this->master_paket_model;
        $data_apbd_model   = $this->data_apbd_model;
        ...
        $breadcrumbs->add('Home', base_url());
        $breadcrumbs->add('Data APBD - Integrasi SIPD', base_url($this->router->fetch_class()));
        ...
        $data['title']                      = "Data APBD - Integrasi SIPD";
        $data['icon']                       = "metismenu-icon fa fa-list-ul";
        $data['description']                = "Mengambil data dari SIPD Kemendagri ........... untuk ditampilkan dan disalin ke dalam aplikasi Simbangda Based Evidence     ";
        $data['breadcrumbs']                = $breadcrumbs->render();
      

        $page                               = 'data_apbd/data_apbd/homepage_integrasi_sipd';
        $data['link']                       = $this->router->fetch_method();
        $data['fetch_method']                       = $this->router->fetch_method();
        $data['menu']                       = $this->load->view('layout/menu', $data, true);
        $data['extra_css']                  = $this->load->view('data_apbd/data_apbd/css', $data, true);
    ...
    public function sub_kegiatan($input_by)
    {
        ...
        $data['dropdown_option']                      = [
            ['tipe'=>'link', 'caption'=>'Kembali', 'fa'=>'fa fa-arrow-left', 'onclick'=>'data_apbd/', 'elemen_tambahan'=>'data-toggle="tooltip" title="Kembali"'],
            // ['tipe'=>'onclick', 'caption'=>'Kegiatan yang sudah di nonaktifkan', 'fa'=>'fa fa-bars', 'onclick'=>"Swal.fire('Error','Fitur ini belum tersedia','error')", 'elemen_tambahan'=>'data-toggle="tooltip" title="Data Sub Kegiatan Yang Sudab Berakhir"'],
            // ['tipe'=>'link', 'caption'=>'Kegiatan yang sudah di nonaktifkan', 'fa'=>'fa fa-bars', 'onclick'=>'data_apbd/kegiatan_berakhir', 'elemen_tambahan'=>'data-toggle="tooltip" title="Data Sub Kegiatan Yang Sudab Berakhir"'],
        ];
    ...
    public function kegiatan_berakhir()
    {
        $breadcrumbs     = $this->breadcrumbs;
        $master_paket   = $this->master_paket_model;
        $data_apbd_model   = $this->data_apbd_model;
        ...
        $data['description']                = "Menampilkan Data APBD";
        $data['breadcrumbs']                = '';
        $data['tot_program']                = $data_apbd_model->total_data_per_instansi_expired(id_instansi())['program'];
        $data['tot_kegiatan']               = $data_apbd_model->total_data_per_instansi_expired(id_instansi())['kegiatan'];
        $data['tot_subkeg']                 = $data_apbd_model->total_data_per_instansi_expired(id_instansi())['subkeg'];
    ...
    public function pengalihan_tahapan_apbd_ski()
    {
        ...
        if (tahapan_apbd()==4) {
            $data['dropdown_option']                      = [
                ...
                ['tipe'=>'link', 'caption'=>'Lihat Data APBD Awal', 'fa'=>'fa fa-bars', 'onclick'=>'lihat_status_data_apbd', 'elemen_tambahan'=>'data-toggle="tooltip" title="DataLihat data APBD Pergeseran"'], 
                ['tipe'=>'link', 'caption'=>'Lihat Data APBD Pergeseran', 'fa'=>'fa fa-bars', 'onclick'=>'lihat_status_data_apbd', 'elemen_tambahan'=>'data-toggle="tooltip" title="DataLihat data APBD Pergeseran"'],
                ['tipe'=>'link', 'caption'=>'Lihat Data APBD Perubahan', 'fa'=>'fa fa-bars', 'onclick'=>'lihat_status_data_apbd', 'elemen_tambahan'=>'data-toggle="tooltip" title="DataLihat data APBD Pergeseran"']
            ];
    ...
    public function setting_apbd_teknis()
    {
        ...
        $tahap = tahapan_apbd();
        $tahun = tahun_anggaran();
        $instansi_teknis = $this->db->get_where('instansi_pembantu_teknis',['id_instansi'=>$id_instansi, 'is_active'=>1]);
        
          $data['dropdown_option']                      = [
            
         
             ['tipe'=>'link', 'caption'=>'Lihat Sub Kegiatan Per Instansi Pembantu', 'fa'=>'fa fa-bars', 'onclick'=>'data_apbd/daftar_kegiatan_berdasarkan_bagian', 'elemen_tambahan'=>'data-toggle="tooltip" title="Lihat Daftar Sub Kegiatan Per Bagian Instansi Pembantu"'],
             ['tipe'=>'link', 'caption'=>'Kembali', 'fa'=>'fa fa-arrow-left', 'onclick'=>'data_apbd/', 'elemen_tambahan'=>'data-toggle="tooltip" title="Kembali"'],
        ];
        ...
        $data['instansi_teknis']                      = $instansi_teknis->result_array() ;
        $data['banyak_instansi_teknis']                      = $instansi_teknis->num_rows() ;
        $data['input_by']                      = "";
        ...
        $count_sub_kegiatan_skpd = $data_apbd_model->total_data_per_instansi(id_instansi(), $tahap, $tahun)['subkeg_skpd'];
         $count_sub_kegiatan_skpd_teknis = $data_apbd_model->total_data_per_instansi(id_instansi(), $tahap, $tahun)['subkeg_teknis'];
        $data['tot_subkeg']                 = $count_sub_kegiatan_skpd;
        $data['tot_subkeg_teknis']                 = $count_sub_kegiatan_skpd_teknis;
        $data['tot_subkeg_semua']                 =$count_sub_kegiatan_skpd + $count_sub_kegiatan_skpd_teknis;
...
public function sync_eplanning()
    ...
    public function dt_program_apbd()
    {
        if(!$this->input->is_ajax_request())
        {
            ...
            foreach ($list as $lists)
            {
                ...
                $kode_bidang_urusan =$lists->kode_bidang_urusan;
                $bidang_urusan = $this->db->query("SELECT nama_bidang_urusan from master_bidang_urusan where kode_bidang_urusan='$kode_bidang_urusan'")->row_array();
                $row    = [];
                $row[]  = $nomor++;
                $row[]  = $bidang_urusan['nama_bidang_urusan'];
                ...
                if ($tahap==4) {
                    ...
                    $q_pagu = $this->db->query("SELECT total_anggaran_program($tahap,$id_instansi,'$kode_program',$tahun) as pagu ")->row_array();
    ...
    public function update_target_keuangan($kode_rekening_sub_kegiatan, $tahap)
    {
        $id_target_apbd = sbe_crypt($this->input->post('pk'), 'D');
                        ...
                        if ($id_kedudukan=='') {
                              $row[]     =  '<div class="btn-group">'.$tombol_copy.$tombolsumber_dana.$tomboltarget.
                                ...
                                <button class="btn btn-info btn-xs"  title="Input anggaran belanja sub  kegiatan '.$lists->nama_sub_kegiatan.'"  onclick="'.$onclick2.'"><i class="fas fa-money-bill"></i></button> '.
                                 ...
                                 '<button class="btn btn-danger btn-xs" onclick="'.$onclick.'"  title="Hapus sub  kegiatan '.$lists->nama_sub_kegiatan.'" ><i class="fa fa-trash"></i></button>
                                </div> ' ;
                        }else{
    ...
    public function sub_kegiatan_apbd_instansi_gabungan($input_by)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                // $row[]  = json_encode($kumpul_usk_pptk);
               ...
               $onclick_copy = "copy_data_apbd_sub_kegiatan('".$lists->kode_rekening_sub_kegiatan."','".$lists->kode_rekening_kegiatan."', '".$lists->kode_rekening_program."','".tahapan_apbd()."','".$lists->kode_bidang_urusan."','".$lists->kode_rekening_sub_kegiatan."')";
                ...
                $row[]     =  "";
                    ...
                    if (jadwal_input_data_dasar()['aktif']==0) {
                        ...
                        $row[]     =  '
                        <div class="btn-group">
                        <button class="btn btn-outline-danger btn-sm" onclick="'.$alert_kunci_input.'">'.jadwal_input_data_dasar()['pesan_action'].'</button>
                        </div> ' ;
                    }else{
                        if ($id_kedudukan=='') {
                              $row[]     =  '<div class="btn-group">'.$tombol.
                                '
                                <button class="btn btn-info btn-xs"  title="Input anggaran belanja sub  kegiatan '.$lists->nama_sub_kegiatan.'"  onclick="'.$onclick2.'"><i class="fas fa-money-bill"></i></button> '.
                                 // '<button class="btn btn-info btn-xs"  title="Input indikator sub kegiatan '.$lists->nama_sub_kegiatan.'"><i class="metismenu-icon fas fa-file-signature"></i></button> ' .
                                 '<button class="btn btn-danger btn-xs" onclick="'.$onclick.'"  title="Hapus sub  kegiatan '.$lists->nama_sub_kegiatan.'" ><i class="fa fa-trash"></i></button>
    ...
    public function pengalihan_apbd_sub_kegiatan_apbd_instansi_gabungan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $id_instansi = id_instansi();
            $kode_kegiatan  = $this->input->post('kode_kegiatan');
            $kode_tahap = [2=>'APBD AWAL',3=>'APBD Pergeseran',4=>'Pengalihan APBD Awal Ke APBD Perubahan'];
            if ($tahap==4) {
                // $where          = 'id_instansi='.id_instansi().' and tahun='.$tahun.' and kode_tahap in (2,3) and status=1';
                $where          = 'id_instansi='.id_instansi().' and tahun='.$tahun.' and kode_tahap in (2,3) and kode_tahap_pengecekan_pengalihan_data !='.$tahap;
            }else {
                $where          = 'id_instansi='.id_instansi().' and tahun='.$tahun.' and kode_tahap in (2) and status=1';
            ...
            foreach ($list as $lists) {
                ...
                $row[]  = '<b>'.$lists->kategori;
                 ...
                 $q_status_pengalihan = $this->db->query("SELECT * from sub_kegiatan_instansi where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap='4' and kode_sub_kegiatan='$krsk'");
                 $cek_status_pengalihan = $q_status_pengalihan->num_rows()==0 ? 'Dinonaktifkan' : 'Dialihkan ke APBD Perubahan';
                $row[]  = $lists->status==1 ? $kode_tahap[$lists->kode_tahap] : $cek_status_pengalihan;
                $row[]  = number_format($pagu);



                $onclick = "alihkan_sub_kegiatan_instansi_apbd_awal_ke_perubahan('".$lists->id_sub_kegiatan_instansi."','".$kode_sub_kegiatan."')";
                $onclick_akhiri = "akhiri_sub_kegiatan_instansi_apbd_awal('".$lists->id_sub_kegiatan_instansi."','".$kode_sub_kegiatan."')";
                $onclick_tetap = "tidak_berubah_sub_kegiatan_instansi_apbd_awal_dan_perubahan('".$lists->id_sub_kegiatan_instansi."','".$kode_sub_kegiatan."')";
               

                if ($lists->status==1) {
                    $row[]     = '
                    <button class="btn btn-warning btn-xs" onclick="'.$onclick.'" data-toggle="tooltip" title="Alihkan Sub Kegiatan ke '.pilihan_nama_tahapan(tahapan_apbd()).'"><i class="fa fa-random"></i></button> 
                    <button class="btn btn-success btn-xs" onclick="'.$onclick_tetap.'" data-toggle="tooltip" title="Data sub kegiatan tidak berubah pada '.pilihan_nama_tahapan(tahapan_apbd()).'"><i class="fa fa-check"></i></button> 
                    <button class="btn btn-danger btn-xs" onclick="'.$onclick_akhiri.'" data-toggle="tooltip" title="Nonaktifkan Sub Kegiatan pada '.pilihan_nama_tahapan(tahapan_apbd()).'"><i class="fa fa-times"></i></button>';
    ...
    public function show_pilihan_sub_kegiatan_apbd()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($tahap==4) {
                ...
                $where          = ['tahun'=>$tahun,'kode_tahap'=>$tahap, 'id_instansi' => id_instansi(), 'kategori'=>'Sub Kegiatan SKPD'];
 ...
 public function copy_data_apbd_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $kode_program = $this->input->post('kode_program');
            $id_table_program = str_replace('.', '-', $kode_program);
            $kode_bidang_urusan = $this->input->post('kode_bidang_urusan');
            $tahap = $this->input->post('tahap');
            $id_instansi = id_instansi();
            $tahun = tahun_anggaran();
            ...
            // $output['data'] = $anggaran_perubahan->result_array();
            ...
            if ($anggaran_perubahan->num_rows()==0) {
               ...
               $output['data']['id_table_program'] = $id_table_program;
    ...
    public function cek_master_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
          ...
          $tahun = tahun_anggaran();
          $id_instansi = id_instansi();
            $subkeg                 = $this->db->query("SELECT kode_sub_kegiatan, nama_sub_kegiatan, kode_bidang_urusan from master_sub_kegiatan where kode_sub_kegiatan='$kode_rekening_sub_kegiatan'")->row();
           
            $output['kode_bidang_urusan']  = $subkeg->kode_bidang_urusan;
            ...
            $ski_uptd = $this->db->query("SELECT * from sub_kegiatan_instansi where kode_sub_kegiatan like '%$kode_rekening_sub_kegiatan%' and tahun = '$tahun' and kode_tahap='$tahap' and id_instansi = '$id_instansi' and kategori='Unit Pelaksana'");
                $output['status'] = true;
            $output['data_teknis_tersimpan']  =$ski_uptd->result_array();


            $instansi_teknis = $this->db->query("SELECT * from instansi_pembantu_teknis where id_instansi = '$id_instansi' and is_active='1' and id_instansi_pembantu_teknis not in (SELECT id_instansi_pembantu_teknis from sub_kegiatan_instansi where tahun='$tahun' and kode_tahap='$tahap' and id_instansi='$id_instansi' and kode_sub_kegiatan like '%$kode_rekening_sub_kegiatan%')
                ");
            $output['instansi_teknis']                      = $instansi_teknis->result_array() ;
            $output['banyak_instansi_teknis']                      = $instansi_teknis->num_rows() ;



            $output['banyak_ski_teknis']  =$ski_uptd->num_rows();
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($status == 'insert') {
                $data = [
                    ...
                    'dbh'                       => $dbh,
                    'lainnya'                   => $lainnya,
                    'id_jenis_sumber_dana'                   => $id_jenis_sumber_dana,
 ...
 public function copy_sub_kegiatan_data_apbd()
    ...
    public function simpan_pengalihan_sub_kegiatan_instansi_apbd_awal_ke_perubahan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $this->db->insert('sub_kegiatan_instansi', $data_ski_baru);
            $this->db->update('sub_kegiatan_instansi', ['status'=>0,'kode_tahap_pengecekan_pengalihan_data'=>tahapan_apbd()], ['id_sub_kegiatan_instansi'=>$id_sub_kegiatan_instansi]);
    ...
    public function tidak_berubah_sub_kegiatan_instansi_apbd_awal_dan_perubahan()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $this->db->update('sub_kegiatan_instansi', ['kode_tahap_pengecekan_pengalihan_data'=>tahapan_apbd()], ['id_sub_kegiatan_instansi'=>$id_sub_kegiatan_instansi]);
            // $this->db->update('anggaran_sub_kegiatan', ['status'=>0], $where_ask_off);
    ...
    public function save_ski_teknis()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run($this)) {
                ...
                $kode_bidang_urusan = $this->db->query("SELECT * from master_sub_kegiatan where kode_sub_kegiatan='$kode_sub_kegiatan'")->row_array()['kode_bidang_urusan'];
                $tahap = $post['tahap'];

                $output['data']['kode_sub_kegiatan'] = $kode_sub_kegiatan ;
                $output['data']['kode_kegiatan'] = $kode_kegiatan     ;
                $output['data']['kode_program'] = $kode_program      ;
                $output['data']['kode_bidang_urusan'] = $kode_bidang_urusan;
                $output['data']['tahap'] = $tahap;



                $id_instansi_teknis           = $post['id_instansi_teknis'];
                $cek_instansi_teknis = $this->db->query("SELECT * from instansi_pembantu_teknis where id_instansi_pembantu_teknis='$id_instansi_teknis'")->row_array();


                $kelompok           = $cek_instansi_teknis['jenis_teknis'];
                $keterangan         = $cek_instansi_teknis['nama_instansi_teknis'];
                $kode               = $cek_instansi_teknis['kode_instansi_teknis'];
                $kode_ski_teknis    = $kode_sub_kegiatan.'.'.$kode;
                 ...
                 $data = [
                            ...
                            'id_instansi_pembantu_teknis'=>$id_instansi_teknis,
                            'id_instansi'=>id_instansi(),
                            'kode_tahap'=>tahapan_apbd(),
                            'tahun'=>tahun_anggaran(),
                            'created_on'=>timestamp(),
...
$kode_rekening_sub_kegiatan = $this->input->post('kode_sub_kegiatan');
    ...
    public function sub_kegiatan_apbd_all()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $column_order   = ['', 'nama_sub_kegiatan'];
            $column_search  = ['nama_sub_kegiatan','kode_sub_kegiatan'];
            $order          = ['nama_sub_kegiatan' => 'ASC'];
            ...
            foreach ($list as $lists) {
                ...
                if ($sub_kegiatan_instansi>0) {
                    ...
                    $onclick = "hapus_sub_kegiatan_instansi('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$data_sub_kegiatan_instansi['kode_tahap']."', '".tahun_anggaran()."','all')";
 ...
 public function hapus_all_apbd_instansi()
    ...
    public function hapus_sub_kegiatan_instansi()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($sub_kegiatan->num_rows() > 0) {
                ...
                $pecah_krsk = explode('.', $kode_sub_kegiatan);
                $output['kode_sub_kegiatan']  = $pecah_krsk[0].'.'.$pecah_krsk[1].'.'.$pecah_krsk[2].'.'.$pecah_krsk[3].'.'.$pecah_krsk[4].'.'.$pecah_krsk[5];
    ...
    public function tambah_sub_kegiatan_instansi()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $tahun = tahun_anggaran();
            $tahap = tahapan_apbd();
            $where      = ['kode_sub_kegiatan' => $kode_sub_kegiatan, 'id_instansi'=>id_instansi(), 'kode_kegiatan'=>$kode_kegiatan, 'kode_program'=>$kode_program, 'tahun'=>$tahun, 'kode_tahap'=>$tahap];
            $sub_kegiatan    = $this->db->get_where('sub_kegiatan_instansi', $where);
    ...
    public function save_master_program()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==3) {
                    $query_cek_program=$this->db->query("SELECT * from master_program where kode_program='$kode' and status !=2 order by id_program desc ");
                    $j_cek_program = $query_cek_program->num_rows();
                    if ($j_cek_program>0) {
                        $data_program = $query_cek_program->row();
                        $status = $data_program->status;
                        $keterangan_status         = ["Pengusulan","Disetujui Admin","Ditolak Admin"];
                        if ($status=='') {
    ...
    public function saveedit_master_program()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==3) {
                    $query_cek_program=$this->db->query("SELECT * from master_program where kode_program='$kode' and status !=2 and id_program !='$id_program' order by id_program desc");
                    $j_cek_program = $query_cek_program->num_rows();
                    if ($j_cek_program>0) {
                        $data_program = $query_cek_program->row();
                        $status = $data_program->status;
    ...
    public function save_master_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==5) {
                    $query_cek_kegiatan=$this->db->query("SELECT * from master_kegiatan where kode_kegiatan='$kode' and status !=2 order by id_kegiatan desc");
    ...
    public function saveedit_master_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==5) {
                    $query_cek_kegiatan=$this->db->query("SELECT * from master_kegiatan where kode_kegiatan='$kode' and status !=2 and id_kegiatan !='$id_kegiatan'  order by id_kegiatan desc");
                    $j_cek_kegiatan = $query_cek_kegiatan->num_rows();
                    if ($j_cek_kegiatan>0) {
                        $data_kegiatan = $query_cek_kegiatan->row();
                        $status = $data_kegiatan->status;
    ...
    public function save_master_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==6) {
                    $query_cek_sub_kegiatan=$this->db->query("SELECT * from master_sub_kegiatan where kode_sub_kegiatan='$kode' and status !=2 order by id_sub_kegiatan desc");
    ...
    public function saveedit_master_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run()) {
                ...
                $j_pecah = count($pecah);
                if ($j_pecah==6) {
                    $query_cek_sub_kegiatan=$this->db->query("SELECT * from master_sub_kegiatan where kode_sub_kegiatan='$kode' and status !=2 and id_sub_kegiatan !='$id_sub_kegiatan'  order by id_sub_kegiatan desc");
                    $j_cek_sub_kegiatan = $query_cek_sub_kegiatan->num_rows();
                    if ($j_cek_sub_kegiatan>0) {
                        $data_sub_kegiatan = $query_cek_sub_kegiatan->row();
                        $status = $data_sub_kegiatan->status;
